<template>
    ...
    <a-table bordered :row-selection="{ selectedRowKeys: selectedRowKeys, onChange: onSelectChange }"
        ...
        <template #bodyCell="{ column, text, record }">
            <template v-if="['plan', 'cost1', 'cost2', 'cost3', 'cost4', 'cost5','assign_cost'].includes(column.dataIndex)">
                <div>
            ...
            if (state.selectedRowKeys.length > 1) {
                ...
                setTimeout(() => {
                    ...
                    dataSource.value.forEach(function (item) {
                        assign_num.push((1-factor.value)*(Math.min(item.cost1, item.cost2, item.cost3, item.cost4, item.cost5))
                        +factor.value*(Math.max(item.cost1, item.cost2, item.cost3, item.cost4, item.cost5)  )            
                        )
                    })
                    let i =0;
                    let temp=0
